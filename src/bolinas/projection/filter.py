"""Post-projection filters shared by MAF and HAL backends.

A query window can project across multiple alignment blocks (MAF) or split
into multiple lift records (halLiftover). ``filter_single_chrom_strand``
groups by ``(query_name, species)``, drops groups that disagree on chrom or
strand, and merges the remaining group into one min/max span. A subsequent
``filter_length`` drops projections outside a sensible length range before
...
def filter_single_chrom_strand(records: pl.DataFrame) -> pl.DataFrame:
    ...
    ``t_end = max(t_end)``, and the unique chrom/strand. Groups whose hits
    span multiple chroms or both strands are dropped.

    Required input columns:
        ``query_name``, ``species``, ``t_chrom``, ``t_start``, ``t_end``,
        ``t_strand``, ``t_src_size``.

    Output columns: same set, one row per surviving group.
    """
    if records.is_empty():

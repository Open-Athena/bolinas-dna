"""4-pass forward-inference + on-the-fly scoring for zero-shot VEP.
...
For each batch we immediately compute:

- **Likelihood scores (6)** from the bidirectional 4-pass conditional
  ``P(center | left, right)``: ``llr``, ``minus_llr``, ``abs_llr``,
  ``minus_logp_ref``, ``minus_logp_alt``, ``entropy``.
- **Embedding scores (24)** = 3 distances × 4 pool strategies × 2 layers,
  computed from the in-batch per-position embeddings for the REF and ALT
  candidates.

Embeddings are **not persisted** — caching the (N, T, D) tensors for the largest
configs would be tens of GB per (model, window, dataset) and a per-cache S3
upload bottleneck. Instead we score in-memory per batch and discard. Output is
a small per-variant DataFrame with the 30 score columns (~MB total per config).
...
# ---------------------------------------------------------------------------
# Combined 4-pass forward + on-the-fly scoring.
# ---------------------------------------------------------------------------
...
def extract_features_and_score(
    ...
    """Run 4-pass inference + score 30 zero-shot VEP rules per variant.

    Each variant contributes 4 forward-pass rows; per-batch input tensor shape
    is ``(4 * batch_size, T)`` where ``T = window_size + n_prefix``. The 4
    per-position embedding tensors (~tens of GB at large configs) live only
    inside the batch — once we've computed embedding distances against the
    REF and ALT candidates, we discard them and move on.

    Args:
        checkpoint_path: HuggingFace checkpoint directory.
        ...
        window_size: DNA window length around the variant.
    ...
    Returns:
        DataFrame of shape ``(N, 30)`` with the columns in :data:`SCORE_NAMES`,
        row-aligned with the input ``dataset``. All values fp32. No NaNs.
    """
    ...
    rope_orig = (
        int(rope_scaling.get("original_max_position_embeddings", max_pos or 0))
        if rope_scaling else (max_pos or 0)
    )
    ...
    # Pre-allocate the per-variant score matrix (30 columns × ~1 MB total).
    out = np.empty((N, len(SCORE_NAMES)), dtype=np.float32)
    ...
    for batch_start in range(0, N, batch_size):
        ...
        # Joint sequence log-prob per candidate sequence: sum per-position
        # log-probs of the input tokens. Reshape (B*4, T-1) → (B, 4).
        logits = model_out.logits  # (B*4, T, V)
        pos_logp = _per_position_logprobs(logits.float(), input_ids_flat)
        seq_logp_b = pos_logp.reshape(B, 4, T_tok - 1).sum(dim=-1)  # (B, 4)
        del logits  # free vocab-sized memory ASAP

        # Per-position embeddings for the 4 candidates from this batch.
        ...
        # Gather REF + ALT embedding slices (drop the other 2 candidate passes).
        ...
        # Compute the 30 scores for this batch.
        ...
        for j, name in enumerate(SCORE_NAMES):
            out[batch_start:batch_end, j] = scores_b[name]

        if batch_start % (batch_size * 50) == 0:
            print(f"[features]   {batch_end}/{N} variants", flush=True)

    df = pd.DataFrame(out, columns=SCORE_NAMES)
    assert not df.isna().any().any(), "scores contain NaN — investigate"
    return df

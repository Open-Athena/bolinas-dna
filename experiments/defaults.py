...
"""
Simplified defaults for DNA experiments in bolinas-dna.

This is a minimal version of Marin's defaults.py that contains only the essential
functions for tokenization and training, without dependencies on other experiments modules.
"""
...
def default_train(
    ...
    Args:
        ...
        use_default_validation: Whether to use default validation sets. Default is False for DNA experiments.
        eval_harness_tasks: List of evaluation harness tasks. Use () or [] to disable (default for DNA)
    ...
    if eval_harness_tasks:
        # NOTE: This requires experiments.evals.task_configs which we don't have
        # For now, we'll just log a warning and skip eval harness
        logger.warning("eval_harness_tasks specified but conversion not available. Skipping eval harness.")
        harness_config = None
    else:
        harness_config = None
    ...
    # Compute number of parameters (simplified version)
    num_params = _compute_num_parameters(model_config, vocab_size)
...
def _compute_num_parameters(model_config: LmConfig, vocab_size: int) -> int:
    """
    Simplified parameter count estimation.
    For more accurate counting, use experiments.llama.compute_num_parameters from full Marin.
    """
    # Basic estimation based on common transformer architecture
    d_model = getattr(model_config, 'hidden_dim', getattr(model_config, 'd_model', 512))
    n_layers = getattr(model_config, 'num_layers', getattr(model_config, 'n_layers', 6))

    # Rough estimate: embedding + layers + output head
    embedding_params = vocab_size * d_model
    layer_params = n_layers * (4 * d_model * d_model)  # Simplified estimate
...
def _prepare_data_config(
    ...
    """
    # For DNA experiments, we typically don't use default validation sets
    validation_sets = {}

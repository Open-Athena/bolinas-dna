"""Iter 19 — targeted bin refinement for complex_traits.

Iter 18 showed the leaks concentrate in specific bins:
- ld_score (20, inf] bucket too wide; pos systematically lower than neg there.
  Fix: split high tail into [20, 50, 100, 500, inf].
- MAF (0, 0.005] bucket: missense pathogenic ultra-rare vs gnomAD-common-edge.
  Fix: optionally split low end into [0, 0.0015, 0.003, 0.005] to force
  tighter rare-variant matching (will drop some positives that can't find
  ultra-rare negatives — accepted trade-off).

Three configs:
  A. Refined ld_score only (MAF unchanged from iter 14)
  B. Refined ld_score + refined MAF low end
  C. iter 14 baseline (for comparison)
"""
import math
...
MAF_BIN_EDGES_BASE = [0.0, 0.005, 0.02, 0.05, 0.2, 0.5]                          # iter 14 default
MAF_BIN_EDGES_REFINED = [0.0, 0.0015, 0.003, 0.005, 0.02, 0.05, 0.2, 0.5]        # split low end
LD_BIN_EDGES_BASE = [0.0, 1.0, 5.0, 20.0, 1e6]                                    # iter 14 default
LD_BIN_EDGES_REFINED = [0.0, 1.0, 5.0, 20.0, 50.0, 100.0, 500.0, 1e6]            # split high tail
...
CONFIGS = {
    "C. iter14 baseline": {
        "MAF": MAF_BIN_EDGES_BASE,
        "ld_score": LD_BIN_EDGES_BASE,
    },
    "A. refined ld_score (split high tail)": {
        "MAF": MAF_BIN_EDGES_BASE,
        "ld_score": LD_BIN_EDGES_REFINED,
    },
    "B. refined ld_score + refined MAF (split both)": {
        "MAF": MAF_BIN_EDGES_REFINED,
        "ld_score": LD_BIN_EDGES_REFINED,
    },
}
...
def run_config(name, cfg):
    print(f"\n========== {name} ==========", flush=True)
    print(f"  MAF edges: {cfg['MAF']}")
    print(f"  ld_score edges: {cfg['ld_score']}")
    Vb = add_bin_open(V, "MAF", cfg["MAF"], "MAF_bin")
    Vb = add_bin_open(Vb, "ld_score", cfg["ld_score"], "ld_score_bin")
    V_m = match_features(
        Vb.filter(pl.col("label")),
        Vb.filter(~pl.col("label")),
        ["tss_dist", "exon_dist", "MAF", "ld_score"],
        COMMON_CAT + ["MAF_bin", "ld_score_bin"],
        k=1,
    )

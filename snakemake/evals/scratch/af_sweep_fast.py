...
for af_min in af_thresholds:
    ...
    log(f"=== AF > {af_min}: {common.height} common variants (filter took {time.time()-t:.1f}s, mem {mem_gb():.2f} GB) ===")
    ...
    log(f"  build_dataset: {time.time()-t:.1f}s, mem {mem_gb():.2f} GB, rows={V.height}, pos={V.filter(pl.col('label')).height}")
    ...
    neg_groups = (
        V.filter(~pl.col("label"))
        .group_by(GROUP_COLS)
        .agg(pl.len().alias("n_neg"))
    )
    joined = pos_groups.join(neg_groups, on=GROUP_COLS, how="left").with_columns(
        pl.col("n_neg").fill_null(0)
    ).with_columns(n_kept=pl.min_horizontal("n_pos", "n_neg"))
    by_subset = joined.group_by("consequence_group").agg(pl.col("n_kept").sum().alias(f"af_{af_min}"))
...
for af in af_thresholds[1:]:
    out = out.join(results_per_subset[af], on="consequence_group", how="full", coalesce=True)

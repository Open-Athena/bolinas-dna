...
rule compute_search_stats:
    ...
    output:
        stats="results/search/{dataset}/search_stats_id{identity}_cov{coverage}.parquet",
    run:
        hits = pl.read_csv(
            input.hits,
            separator="\t",
...
rule aggregate_search_stats:
    ...
    input:
        stats=expand(
            "results/search/{dataset}/search_stats_id{identity}_cov{coverage}.parquet",
...
rule plot_search_train_matches_median:
    """Heatmap of median train matches per val sequence (search-based)."""
    input:
        summary="results/search/search_summary.parquet",
    output:
        plot="results/plots/search_train_matches_median.svg",
    run:
        _plot_train_matches(input.summary, output.plot, "train_matches_median", "Median (search)", ".0f")


rule plot_search_train_matches_mean:
    """Heatmap of mean train matches per val sequence (search-based)."""
    input:
        summary="results/search/search_summary.parquet",
    output:

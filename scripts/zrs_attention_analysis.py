...
import io, boto3, numpy as np, torch
import polars as pl
import matplotlib
matplotlib.use("Agg")
...
BUCKET = "oa-bolinas"; PFX = "snakemake/enhancer_classification/"
...
ckpt_path = fetch("results/segmentation/model/xfmr2_w64k_s42/seg_v1_64k/best.ckpt", "/tmp/w64k_best.ckpt")
...
def patched_forward(self, x, attention_bias, compute_dtype=None):
    ...
    B, S, D = x.shape
    if compute_dtype is None: compute_dtype = x.dtype
    ...
    q = apply_rope(q, inplace=True); k = apply_rope(k, inplace=True)
    q_t = q.permute(0, 2, 1, 3); k_t = k.permute(0, 2, 1, 3)
    att = torch.matmul(q_t, k_t.transpose(-2, -1)).float() / math.sqrt(128.0)
    if attention_bias is not None: att = att + attention_bias.float()
...
# --- annotation tracks ------------------------------------------------------
def s3_pq(key):
    return pl.read_parquet(io.BytesIO(s3.get_object(Bucket=BUCKET, Key=PFX+key)["Body"].read())).to_pandas()

# All cCREs (any class) intersecting the window
cre_all = s3_pq("results/cre/homo_sapiens/all.parquet")
cre_in = cre_all[(cre_all["chrom"]=="7") & (cre_all["end"]>WIN_START) & (cre_all["start"]<WIN_END)].copy()
...
els_cons = s3_pq("results/cre/homo_sapiens/noexon/conserved/phastCons_43p/20/ELS.parquet")
els_in = els_cons[(els_cons["chrom"]=="7") & (els_cons["end"]>WIN_START) & (els_cons["start"]<WIN_END)].copy()
els_in["start"] = els_in["start"].clip(lower=WIN_START); els_in["end"] = els_in["end"].clip(upper=WIN_END)
print(f"conserved ELS in window: {len(els_in)}")

# Exons
exons = s3_pq("results/annotation/homo_sapiens/exons.parquet")
ex_in = exons[(exons["chrom"]=="7") & (exons["end"]>WIN_START) & (exons["start"]<WIN_END)].copy()
ex_in["start"] = ex_in["start"].clip(lower=WIN_START); ex_in["end"] = ex_in["end"].clip(upper=WIN_END)
...
# phastCons (43-primate) — pull per-bp values via presigned HTTP URL so we
# don't have to download the 6.9 GB bigwig. Threshold 0.961 is the one used
# for the conserved-ELS filter in the data pipeline (see config.yaml).
import pyBigWig
bw_path = fetch("results/conservation/homo_sapiens/phastCons_43p.bw", "/tmp/phastCons_43p.bw")
bw = pyBigWig.open(bw_path)
PHASTCONS_THR = 0.961
# bigwig uses UCSC chrom naming
phastcons_bp = np.asarray(bw.values("chr7", WIN_START, WIN_END, numpy=True), dtype=np.float32)
bw.close()
...
print(f"phastCons mean: {phastcons_mean.mean():.3f}; conserved-bp fraction: {phastcons_consfrac.mean():.3f}")
...
outer = fig.add_gridspec(
    ...
    top=1 - pad_top / fig_h, bottom=pad_bot / fig_h,
)
# Tracks inner grid
...
for cls in cre_classes:
    axc = fig.add_subplot(gs_tr[trow, 0], sharex=ax0); trow += 1
    sub = cre_in[cre_in["cre_class"] == cls]
    for _, r in sub.iterrows():
        axc.add_patch(Rectangle((r["start"], 0), r["end"] - r["start"], 1,
                                color=class_to_color[cls], lw=0))
    axc.set_yticks([]); axc.set_ylim(0, 1)
    axc.set_ylabel(cls, rotation=0, ha="right", va="center", labelpad=8)
    axc.tick_params(labelbottom=False)

axe = fig.add_subplot(gs_tr[trow, 0], sharex=ax0); trow += 1
for _, r in ex_in.iterrows():
    axe.add_patch(Rectangle((r["start"], 0), r["end"] - r["start"], 1, color="firebrick", lw=0))
axe.set_yticks([]); axe.set_ylim(0, 1)
axe.set_ylabel("exon", rotation=0, ha="right", va="center", labelpad=8)
axe.tick_params(labelbottom=False)

axr = fig.add_subplot(gs_tr[trow, 0], sharex=ax0); trow += 1
...
axp = fig.add_subplot(gs_tr[trow, 0], sharex=ax0); trow += 1
axp.fill_between(xs_bin_centers, 0, phastcons_consfrac, color="seagreen", alpha=0.7, lw=0)
axp.set_ylim(0, 1)
axp.set_ylabel(f"phastCons\nfrac≥{PHASTCONS_THR}", rotation=0, ha="right", va="center", labelpad=8)
...
for li, attn in enumerate(captured):
    ...
    im = ax.imshow(np.log10(a + 1e-9), aspect="auto",
...
print(f"per-bin annotation rates: cre={cre_mask.mean():.3f}  els={els_mask.mean():.3f}  exon={exon_mask.mean():.3f}")

# Incoming attention per key per layer: average over heads then over queries.
incoming = np.stack([attn[0].mean(0).numpy().mean(0) for attn in captured])  # (n_layers, NUM_BINS)
...
print(f"{'variable':28s}  " + "  ".join(f"L{l} rho   p" for l in range(n_layers)))
...
for li in range(n_layers):
    ...
    bars = ax2.bar(x + offset, rhos[li], width,
                   label=f"layer {li}", color=colors[li], edgecolor="black", lw=0.4)
...
ax2.set_title("Per-bin correlations with incoming attention\n(ZRS 64k window; * = p<0.05)")
